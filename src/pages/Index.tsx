  ...
  return (
    <Layout>
      ...
      <div className="bg-[hsl(220,20%,7%)] min-h-screen">
        ...
        {/* ───── PROGRAMS & FUNDING — Venture North Accelerator Style (Light) ───── */}
        <section className="relative py-24 md:py-32 overflow-hidden" style={{ background: 'hsl(220 15% 92%)' }}>
          <img src={linesTeal} alt="" aria-hidden="true" className="pointer-events-none absolute -right-16 top-8 w-[420px] opacity-[0.12] hidden md:block" />
          <img src={signatureLines} alt="" aria-hidden="true" className="pointer-events-none absolute -left-20 bottom-0 w-[360px] opacity-[0.06] hidden md:block" />
          <div className="absolute top-1/3 right-0 w-[500px] h-[500px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(circle, hsl(168 100% 35% / 0.08) 0%, transparent 60%)' }} />

          <div className="container mx-auto px-6 relative z-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 md:mb-16">
              <ScrollReveal className="max-w-2xl">
                <p className="inline-flex items-center gap-2 text-xs font-bold tracking-[0.25em] uppercase mb-5" style={{ color: 'hsl(168, 100%, 28%)' }}>
                  <span className="size-1.5 rounded-full" style={{ background: 'hsl(168, 100%, 28%)' }} />
                  Programs & Funding
                </p>
                <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl leading-[1.05]" style={{ fontFamily: "'Open Sans', sans-serif", fontWeight: 500, color: 'hsl(220, 15%, 20%)' }}>
                  One ecosystem.<br />
                  <span style={{ color: 'hsl(168, 100%, 28%)', fontWeight: 700 }}>Three ways to grow.</span>
                </h2>
              </ScrollReveal>
              <ScrollReveal delay={0.1}>
                <Link to="/apply" className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-sm border-2 transition-all hover:scale-[1.03]" style={{ borderColor: 'hsl(168, 100%, 28%)', color: 'hsl(168, 100%, 28%)' }}>
              ...
              {[
                ...
                <ScrollReveal key={program.title} delay={i * 0.1}>
                  <Link to={program.link} className="group block h-full">
                    <article className="relative h-full overflow-hidden rounded-2xl border transition-all duration-300 hover:-translate-y-1" style={{ borderColor: 'hsl(220 15% 85%)', background: 'white' }}>
                      ...
                      <div className="p-7 -mt-20 relative">
                        ...
                        <h3 className="text-2xl mb-3 uppercase" style={{ fontFamily: "'Open Sans', sans-serif", fontWeight: 700, letterSpacing: '-0.01em', color: 'hsl(220, 15%, 20%)' }}>
                          {program.title}
                        </h3>
                        <p className="text-sm font-light leading-relaxed mb-5" style={{ color: 'hsl(220, 15%, 40%)' }}>
                          {program.description}
                        </p>
                        <span className="inline-flex items-center gap-2 text-sm font-semibold" style={{ color: 'hsl(168, 100%, 28%)' }}>
                          Learn more
                          <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                        </span>
                      </div>
            ...
            <div className="mt-14 md:mt-16 grid grid-cols-1 sm:grid-cols-3 gap-6">
              {[
                ...
                <ScrollReveal key={s.label} delay={i * 0.1}>
                  <div className="border-l-2 pl-4" style={{ borderColor: 'hsl(168, 100%, 35%)' }}>
                    <p className="text-3xl md:text-4xl" style={{ color: 'hsl(168, 100%, 28%)', fontFamily: "'Open Sans', sans-serif", fontWeight: 800 }}>{s.stat}</p>
                    <p className="text-xs mt-1 leading-tight" style={{ color: 'hsl(220, 15%, 40%)' }}>{s.label}</p>
                  </div>
                </ScrollReveal>
              ))}
            </div>
